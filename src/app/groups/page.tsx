...
import { ArrowLeft, Search, Users, FolderPlus, Plus, ChevronRight } from 'lucide-react';
    ...
    <div className="flex flex-col h-screen max-w-md mx-auto border shadow-2xl bg-slate-50 dark:bg-slate-900 relative">
      <header className="p-3 bg-white dark:bg-slate-800 border-b dark:border-slate-700 flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-2">
          <button onClick={() => router.push('/')} className="p-2 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full">
      ...
      {/* --- GRID 2 BAGIAN: KIRI (GRUP PUSAT) & KANAN (TAMBAH SUB-GRUP) --- */}
      ...
      <div className="p-3 bg-white dark:bg-slate-800 border-b dark:border-slate-700">
        <div className="relative flex items-center">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 text-xs bg-slate-100 dark:bg-slate-900 border dark:border-slate-700 rounded-xl text-gray-800 dark:text-gray-100"
          />
        </div>
      </div>
        ...
        {filteredGroups.length === 0 ? (
          ...
          filteredGroups.map((group) => (
            <div
              ...
              className="flex items-center gap-3 p-3 bg-white dark:bg-slate-800 hover:bg-slate-50 rounded-xl border dark:border-slate-700 cursor-pointer shadow-xs"
            >
